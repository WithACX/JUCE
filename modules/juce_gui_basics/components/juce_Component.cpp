...
class HierarchyChecker {
 public:
  ...
  bool shouldBailOut() const { return nearestNonNullParent() == nullptr; }
...
class Component::MouseListenerList {
 public:
  ...
  void addListener(MouseListener* newListener,
  ...
  void removeListener(MouseListener* listenerToRemove) {
    ...
    if (index >= 0) {
      if (index < numDeepMouseListeners) --numDeepMouseListeners;
  ...
  static void sendMouseEvent(HierarchyChecker& checker,
                             EventMethod&& eventMethod, Params&&... params) {
    const auto callListeners = [&](auto& parentComp,
                                   const auto findNumListeners) {
      if (auto* list = parentComp.mouseListeners.get()) {
        const WeakReference safePointer{&parentComp};

        for (int i = findNumListeners(*list); --i >= 0;
             i = jmin(i, findNumListeners(*list))) {
          (list->listeners.getUnchecked(i)->*eventMethod)(
              checker.eventWithNearestParent(), params...);

          if (checker.shouldBailOut() || safePointer == nullptr) return false;
        }
      }
    ...
    if (auto* parent = checker.nearestNonNullParent())
      if (!callListeners(*parent,
                         [](auto& list) { return list.listeners.size(); }))
        return;

    if (auto* parent = checker.nearestNonNullParent())
      for (Component* p = parent->parentComponent; p != nullptr;
           p = p->parentComponent)
        if (!callListeners(
                *p, [](auto& list) { return list.numDeepMouseListeners; }))
          return;
  }
...
//==============================================================================
Component::Component() noexcept : componentFlags(0) {}

Component::Component(const String& name) noexcept
    : componentName(name), componentFlags(0) {}

Component::~Component() {
  static_assert(sizeof(flags) <= sizeof(componentFlags),
                "componentFlags has too many bits!");

  componentListeners.call(
      [this](ComponentListener& l) { l.componentBeingDeleted(*this); });
  ...
  if (parentComponent != nullptr)
    parentComponent->removeChildComponent(
        parentComponent->childComponentList.indexOf(this), true, false);
  else
    giveAwayKeyboardFocusInternal(isParentOf(currentlyFocusedComponent));

  if (flags.hasHeavyweightPeerFlag) removeFromDesktop();

  // Something has added some children to this component during its destructor!
  // Not a smart idea!
...
void Component::setName(const String& name) {
  ...
  if (componentName != name) {
    ...
    if (flags.hasHeavyweightPeerFlag)
      if (auto* peer = getPeer()) peer->setTitle(name);

    BailOutChecker checker(this);
    componentListeners.callChecked(checker, [this](ComponentListener& l) {
      l.componentNameChanged(*this);
    });
  }
}

void Component::setComponentID(const String& newID) { componentID = newID; }
  ...
  if (flags.visibleFlag != shouldBeVisible) {
    ...
    if (!shouldBeVisible) {
      ...
      if (hasKeyboardFocus(true)) {
        if (parentComponent != nullptr) parentComponent->grabKeyboardFocus();
...
void Component::sendVisibilityChangeMessage() {
  ...
  if (!checker.shouldBailOut())
    componentListeners.callChecked(checker, [this](ComponentListener& l) {
      l.componentVisibilityChanged(*this);
    });
}

bool Component::isShowing() const {
  if (!flags.visibleFlag) return false;

  if (parentComponent != nullptr) return parentComponent->isShowing();

  if (auto* peer = getPeer()) return !peer->isMinimised();
...
void* Component::getWindowHandle() const {
  if (auto* peer = getPeer()) return peer->getNativeHandle();
...
void Component::addToDesktop(int styleWanted, void* nativeWindowToAttachTo) {
  ...
  if (peer == nullptr || styleWanted != peer->getStyleFlags()) {
    ...
    setSize(jmax(1, getWidth()), jmax(1, getHeight()));
#endif

    const auto unscaledPosition =
        detail::ScalingHelpers::scaledScreenPosToUnscaled(getScreenPosition());
    const auto topLeft = detail::ScalingHelpers::unscaledScreenPosToScaled(
    ...
    if (peer != nullptr) {
      ...
      internalHierarchyChanged();  // give comps a chance to react to the peer
                                   // change before the old peer is deleted.

      if (safePointer == nullptr) return;

      setTopLeftPosition(topLeft);
    }

    if (parentComponent != nullptr) parentComponent->removeChildComponent(this);
    ...
    if (safePointer != nullptr) {
      ...
      if (wasMinimised) peer->setMinimised(true);

#if JUCE_WINDOWS
      if (isAlwaysOnTop()) peer->setAlwaysOnTop(true);
#endif
...
#if JUCE_LINUX
      // Creating the peer Image on Linux will change the reported position of
      // the window. If the Image creation is interleaved with the coming
      // configureNotifyEvents the window will appear in the wrong position. To
      // avoid this, we force the Image creation here, before handling any of
      // the configureNotifyEvents. The Linux implementation of
      // performAnyPendingRepaintsNow() will force update the peer position if
      // necessary.
      ...
      if (auto* handler = getAccessibilityHandler())
        detail::AccessibilityHelpers::notifyAccessibilityEvent(
            *handler, detail::AccessibilityHelpers::Event::windowOpened);
    }
  }
}
...
void Component::removeFromDesktop() {
  ...
  if (flags.hasHeavyweightPeerFlag) {
    if (auto* handler = getAccessibilityHandler())
      detail::AccessibilityHelpers::notifyAccessibilityEvent(
          *handler, detail::AccessibilityHelpers::Event::windowClosed);
...
ComponentPeer* Component::getPeer() const {
  if (flags.hasHeavyweightPeerFlag) return ComponentPeer::getPeerFor(this);

  if (parentComponent == nullptr) return nullptr;

  return parentComponent->getPeer();
}

void Component::userTriedToCloseWindow() {
  /* This means that the user's trying to get rid of your window with the 'close
     window' system menu option (on windows) or possibly the task manager - you
     should really handle this and delete or hide your component in an
     appropriate way.

     If you want to ignore the event and don't want to trigger this assertion,
     just override this method and do nothing.
  */
  jassertfalse;
}
  ...
  if (shouldBeOpaque != flags.opaqueFlag) {
    ...
    if (flags.hasHeavyweightPeerFlag)
      if (auto* peer = ComponentPeer::getPeerFor(this))
        addToDesktop(
            peer->getStyleFlags());  // recreates the heavyweight window

    repaint();
  }
}

bool Component::isOpaque() const noexcept { return flags.opaqueFlag; }
  ...
  void paint(Graphics& g) override {
    ...
    if (image.isNull() || image.getBounds() != imageBounds) {
      image = Image(owner.isOpaque() ? Image::RGB : Image::ARGB,
                    ...
                    jmax(1, imageBounds.getHeight()), !owner.isOpaque());
    ...
    if (!validArea.containsRectangle(compBounds)) {
      ...
      for (auto& i : validArea) lg.excludeClipRectangle(i);
    ...
    g.drawImageTransformed(
        ...
        AffineTransform::scale(
            (float)compBounds.getWidth() / (float)imageBounds.getWidth(),
...
void Component::setBufferedToImage(bool shouldBeBuffered) {
  ...
  // CachedComponentImage, so by calling setBufferedToImage, you'll be deleting
  // the custom one - this is almost certainly not what you wanted to happen...
  // If you really do know what you're doing here, and want to avoid this
  // assertion, just call setCachedComponentImage (nullptr) before
  // setBufferedToImage().
  jassert(cachedImage == nullptr || dynamic_cast<StandardCachedComponentImage*>(
                                        cachedImage.get()) != nullptr);
  ...
  if (other != nullptr && other != this) {
    ...
    if (parentComponent != nullptr) {
      ...
      if (index >= 0 && childList[index + 1] != other) {
        ...
        if (otherIndex >= 0) {
          if (index < otherIndex) --otherIndex;
      ...
      if (other->isOnDesktop()) {
        ...
        if (us != nullptr && them != nullptr) us->toBehind(them);
      }
    }
  }
}
...
void Component::setAlwaysOnTop(bool shouldStayOnTop) {
  if (shouldStayOnTop != flags.alwaysOnTopFlag) {
    ...
    if (shouldStayOnTop && !checker.shouldBailOut()) toFront(false);

    if (!checker.shouldBailOut()) internalHierarchyChanged();
  }
}

bool Component::isAlwaysOnTop() const noexcept { return flags.alwaysOnTopFlag; }

//==============================================================================
int Component::proportionOfWidth(float proportion) const noexcept {
  return roundToInt(proportion * (float)boundsRelativeToParent.getWidth());
}
int Component::proportionOfHeight(float proportion) const noexcept {
...
Point<int> Component::getLocalPoint(const Component* source,
                                    Point<int> point) const {
  return detail::ComponentHelpers::convertCoordinate(this, source, point);
}
Point<float> Component::getLocalPoint(const Component* source,
                                      Point<float> point) const {
  return detail::ComponentHelpers::convertCoordinate(this, source, point);
}
Rectangle<int> Component::getLocalArea(const Component* source,
                                       Rectangle<int> area) const {
  return detail::ComponentHelpers::convertCoordinate(this, source, area);
}
Rectangle<float> Component::getLocalArea(const Component* source,
                                         Rectangle<float> area) const {
  return detail::ComponentHelpers::convertCoordinate(this, source, area);
}

Point<int> Component::localPointToGlobal(Point<int> point) const {
  return detail::ComponentHelpers::convertCoordinate(nullptr, this, point);
}
...
Rectangle<int> Component::localAreaToGlobal(Rectangle<int> area) const {
  return detail::ComponentHelpers::convertCoordinate(nullptr, this, area);
}
Rectangle<float> Component::localAreaToGlobal(Rectangle<float> area) const {
  return detail::ComponentHelpers::convertCoordinate(nullptr, this, area);
}
...
void Component::setBounds(int x, int y, int w, int h) {
  ...
  if (wasMoved || wasResized) {
    ...
    if (showing) {
      ...
      if (!flags.hasHeavyweightPeerFlag) repaintParent();
    }
    ...
    if (flags.hasHeavyweightPeerFlag)
      if (auto* peer = getPeer()) peer->updateBounds();
...
void Component::sendMovedResizedMessages(bool wasMoved, bool wasResized) {
  ...
  if (parentComponent != nullptr) parentComponent->childBoundsChanged(this);

  if (!checker.shouldBailOut()) {
    componentListeners.callChecked(
        checker, [this, wasMoved, wasResized](ComponentListener& l) {
...
void Component::setCentrePosition(Point<int> p) {
...
void Component::setBoundsRelative(Rectangle<float> target) {
  setBounds(
      (target * Point<float>((float)getParentWidth(), (float)getParentHeight()))
          .toNearestInt());
}
...
void Component::centreWithSize(int width, int height) {
  auto parentArea =
      detail::ComponentHelpers::getParentOrMainMonitorBounds(*this)
          .transformedBy(getTransform().inverted());

  setBounds(parentArea.getCentreX() - width / 2,
            parentArea.getCentreY() - height / 2, width, height);
}

void Component::setBoundsInset(BorderSize<int> borders) {
  setBounds(borders.subtractedFrom(
      detail::ComponentHelpers::getParentOrMainMonitorBounds(*this)));
}
...
void Component::setBoundsToFit(Rectangle<int> targetArea,
  ...
  if (onlyReduceInSize && getWidth() <= targetArea.getWidth() &&
    ...
    if (sourceRatio <= targetRatio)
      sourceArea.setHeight(
          jmin(targetArea.getHeight(),
               roundToInt(targetArea.getWidth() * sourceRatio)));
    else
      sourceArea.setWidth(
          jmin(targetArea.getWidth(),
...
void Component::setTransform(const AffineTransform& newTransform) {
  // If you pass in a transform with no inverse, the component will have no
  // dimensions, and there will be all sorts of maths errors when converting
  // coordinates.
...
float Component::getApproximateScaleFactorForComponent(
    const Component* targetComponent) {
  AffineTransform transform;

  for (auto* target = targetComponent; target != nullptr;
...
bool Component::hitTest(int x, int y) {
  if (!flags.ignoresMouseClicksFlag) return true;
  ...
  if (flags.allowChildMouseClicksFlag) {
    for (int i = childComponentList.size(); --i >= 0;) {
      ...
      if (child.isVisible() &&
          detail::ComponentHelpers::hitTest(
              child, detail::ComponentHelpers::convertFromParentSpace(
                         child, Point<int>(x, y).toFloat())))
        return true;
    }
  }
...
void Component::setInterceptsMouseClicks(
    bool allowClicks, bool allowClicksOnChildComponents) noexcept {
  flags.ignoresMouseClicksFlag = !allowClicks;
  flags.allowChildMouseClicksFlag = allowClicksOnChildComponents;
}

void Component::getInterceptsMouseClicks(
...
bool Component::contains(Point<int> point) { return contains(point.toFloat()); }

bool Component::contains(Point<float> point) {
  if (detail::ComponentHelpers::hitTest(*this, point)) {
    if (parentComponent != nullptr)
      return parentComponent->contains(
          detail::ComponentHelpers::convertToParentSpace(*this, point));

    if (flags.hasHeavyweightPeerFlag)
      if (auto* peer = getPeer())
        return peer->contains(
            detail::ComponentHelpers::localPositionToRawPeerPos(*this, point)
                .roundToInt(),
            true);
  }
...
bool Component::reallyContains(Point<int> point,
                               bool returnTrueIfWithinAChild) {
  return reallyContains(point.toFloat(), returnTrueIfWithinAChild);
}

bool Component::reallyContains(Point<float> point,
                               bool returnTrueIfWithinAChild) {
  ...
  return (compAtPosition == this) ||
         (returnTrueIfWithinAChild && isParentOf(compAtPosition));
}
...
Component* Component::getComponentAt(Point<float> position) {
  if (flags.visibleFlag && detail::ComponentHelpers::hitTest(*this, position)) {
    for (int i = childComponentList.size(); --i >= 0;) {
      ...
      child = child->getComponentAt(
          detail::ComponentHelpers::convertFromParentSpace(*child, position));
...
void Component::addChildComponent(Component& child, int zOrder) {
  ...
  if (child.parentComponent != this) {
    ...
    if (child.isVisible()) child.repaintParent();
...
void Component::addChildComponent(Component* child, int zOrder) {
  if (child != nullptr) addChildComponent(*child, zOrder);
}

void Component::addAndMakeVisible(Component* child, int zOrder) {
  if (child != nullptr) addAndMakeVisible(*child, zOrder);
}
...
Component* Component::removeChildComponent(int index, bool sendParentEvents,
  ...
  if (auto* child = childComponentList[index]) {
    ...
    // (NB: there are obscure situations where child->isShowing() = false, but
    // it still has the focus)
    if (child->hasKeyboardFocus(true)) {
      const WeakReference<Component> safeThis(this);

      child->giveAwayKeyboardFocusInternal(sendChildEvents ||
                                           currentlyFocusedComponent != child);

      if (sendParentEvents) {
        if (safeThis == nullptr) return child;

        grabKeyboardFocus();
      }
    }

    if (sendChildEvents) child->internalHierarchyChanged();

    if (sendParentEvents) internalChildrenChanged();
...
Component* Component::findChildWithID(StringRef targetID) const noexcept {
  for (auto* c : childComponentList)
    if (c->componentID == targetID) return c;
...
Component* Component::getTopLevelComponent() const noexcept {
  ...
  while (comp->parentComponent != nullptr) comp = comp->parentComponent;
...
bool Component::isParentOf(const Component* possibleChild) const noexcept {
  while (possibleChild != nullptr) {
    ...
    if (possibleChild == this) return true;
  }
...
void Component::internalChildrenChanged() {
  if (componentListeners.isEmpty()) {
    ...
    if (!checker.shouldBailOut())
      componentListeners.callChecked(checker, [this](ComponentListener& l) {
        l.componentChildrenChanged(*this);
      });
  }
}
...
void Component::internalHierarchyChanged() {
  ...
  componentListeners.callChecked(checker, [this](ComponentListener& l) {
    l.componentParentHierarchyChanged(*this);
  });
  ...
  for (int i = childComponentList.size(); --i >= 0;) {
    ...
    if (checker.shouldBailOut()) {
      // you really shouldn't delete the parent component during a callback
      // telling you that it's changed..
      jassertfalse;
      return;
    }
  ...
  if (!MessageManager::getInstance()->isThisTheMessageThread()) {
    ...
    return (int)(pointer_sized_int)MessageManager::getInstance()
        ->callFunctionOnMessageThread(
            &detail::ComponentHelpers::runModalLoopCallback, this);
  }

  if (!isCurrentlyModal(false)) enterModalState(true);

  return ModalComponentManager::getInstance()
...
void Component::enterModalState(bool shouldTakeKeyboardFocus,
  ...
  if (!isCurrentlyModal(false)) {
    // While this component is in modal state it may block other components from
    // receiving mouseExit events. To keep mouseEnter and mouseExit calls
    // balanced on these components, we must manually force the mouse to "leave"
    // blocked components.
    detail::ComponentHelpers::sendMouseEventToComponentsThatAreBlockedByModal(
        *this, &Component::internalMouseExit);

    if (safeReference == nullptr) {
      // If you hit this assertion, the mouse-exit event above has caused the
      // modal component to be deleted.
      jassertfalse;
      return;
    }
    ...
    if (shouldTakeKeyboardFocus) grabKeyboardFocus();
  } else {
...
void Component::exitModalState(int returnValue) {
  ...
  if (isCurrentlyModal(false)) {
    if (MessageManager::getInstance()->isThisTheMessageThread()) {
      ...
      // While this component is in modal state it may block other components
      // from receiving mouseEnter events. To keep mouseEnter and mouseExit
      // calls balanced on these components, we must manually force the mouse to
      // "enter" blocked components.
      if (deletionChecker != nullptr)
        detail::ComponentHelpers::
            sendMouseEventToComponentsThatAreBlockedByModal(
                *deletionChecker, &Component::internalMouseEnter);
    } else {
      MessageManager::callAsync(
          [target = WeakReference<Component>{this}, returnValue] {
            if (target != nullptr) target->exitModalState(returnValue);
          });
    }
  }
}

bool Component::isCurrentlyModal(
    bool onlyConsiderForemostModalComponent) const noexcept {
  auto& mcm = *ModalComponentManager::getInstance();
...
bool Component::isCurrentlyBlockedByAnotherModalComponent() const {
  return detail::ComponentHelpers::modalWouldBlockComponent(
      *this, getCurrentlyModalComponent());
}
...
Component* JUCE_CALLTYPE
Component::getCurrentlyModalComponent(int index) noexcept {
  return ModalComponentManager::getInstance()->getModalComponent(index);
}

//==============================================================================
void Component::setBroughtToFrontOnMouseClick(
...
void Component::setMouseCursor(const MouseCursor& newCursor) {
  if (cursor != newCursor) {
    ...
    if (flags.visibleFlag) updateMouseCursor();
  }
}

MouseCursor Component::getMouseCursor() { return cursor; }
...
void Component::setAlpha(float newAlpha) {
  auto newIntAlpha =
      (uint8)(255 - jlimit(0, 255, roundToInt(newAlpha * 255.0)));
...
void Component::alphaChanged() {
  if (flags.hasHeavyweightPeerFlag) {
    if (auto* peer = getPeer()) peer->setAlpha(getAlpha());
  } else {
    repaint();
  }
}
...
//==============================================================================
void Component::repaint() { internalRepaintUnchecked(getLocalBounds(), true); }

void Component::repaint(int x, int y, int w, int h) {
  internalRepaint({x, y, w, h});
}

void Component::repaint(Rectangle<int> area) { internalRepaint(area); }

void Component::repaintParent() {
  if (parentComponent != nullptr)
    parentComponent->internalRepaint(
        detail::ComponentHelpers::convertToParentSpace(*this,
                                                       getLocalBounds()));
}
...
void Component::internalRepaint(Rectangle<int> area) {
  ...
  if (!area.isEmpty()) internalRepaintUnchecked(area, false);
}

void Component::internalRepaintUnchecked(Rectangle<int> area,
  ...
  if (flags.visibleFlag) {
    ...
    if (flags.hasHeavyweightPeerFlag) {
      if (auto* peer = getPeer()) {
        // Tweak the scaling so that the component's integer size exactly aligns
        // with the peer's scaled size
        auto peerBounds = peer->getBounds();
        auto scaled =
            area *
            Point<float>((float)peerBounds.getWidth() / (float)getWidth(),
                         (float)peerBounds.getHeight() / (float)getHeight());

        peer->repaint(affineTransform != nullptr
                          ? scaled.transformedBy(*affineTransform)
                          : scaled);
      }
    } else {
      if (parentComponent != nullptr)
        parentComponent->internalRepaint(
            detail::ComponentHelpers::convertToParentSpace(*this, area));
    }
  }
}
...
void Component::paintComponentAndChildren(Graphics& g) {
  ...
  if (flags.dontClipGraphicsFlag && getNumChildComponents() == 0) {
    ...
    if (!(detail::ComponentHelpers::clipObscuredRegions(*this, g, clipBounds,
                                                        {}) &&
          g.isClipEmpty()))
      paint(g);
  }
        ...
        if (child.flags.dontClipGraphicsFlag) {
          ...
          for (int j = i + 1; j < childComponentList.size(); ++j) {
            ...
            if (sibling.flags.opaqueFlag && sibling.isVisible() &&
                sibling.affineTransform == nullptr) {
...
void Component::paintEntireComponent(Graphics& g, bool ignoreAlphaLevel) {
  // If sizing a top-level-window and the OS paint message is delivered
  // synchronously before resized() is called, then we'll invoke the callback
  // here, to make sure the components inside have had a chance to sort their
  // sizes out..
#if JUCE_DEBUG
  if (!flags.isInsidePaintCall)  // (avoids an assertion in plugins hosted in
                                 // WaveLab)
#endif
    sendMovedResizedMessagesIfPending();
  ...
  if (effect != nullptr) {
                      ...
                      !flags.opaqueFlag);
    {
      ...
      g2.addTransform(AffineTransform::scale(
          (float)scaledBounds.getWidth() / (float)getWidth(),
    ...
    effect->applyEffect(effectImage, g, scale,
                        ignoreAlphaLevel ? 1.0f : getAlpha());
...
void Component::setPaintingIsUnclipped(
...
Image Component::createComponentSnapshot(Rectangle<int> areaToGrab,
  ...
  if (clipImageToComponentBounds) r = r.getIntersection(getLocalBounds());
...
LookAndFeel& Component::getLookAndFeel() const noexcept {
  for (auto* c = this; c != nullptr; c = c->parentComponent)
    if (auto lf = c->lookAndFeel.get()) return *lf;
...
Colour Component::findColour(int colourID, bool inheritFromParent) const {
  if (auto* v = properties.getVarPointer(
          detail::ComponentHelpers::getColourPropertyID(colourID)))
...
void Component::setColour(int colourID, Colour colour) {
  if (properties.set(detail::ComponentHelpers::getColourPropertyID(colourID),
                     (int)colour.getARGB()))
    colourChanged();
}
...
void Component::copyAllExplicitColoursTo(Component& target) const {
  ...
  for (int i = properties.size(); --i >= 0;) {
    ...
    if (name.toString().startsWith(detail::colourPropertyPrefix))
      if (target.properties.set(name, properties[name])) changed = true;
  }

  if (changed) target.colourChanged();
}
...
void Component::mouseWheelMove(const MouseEvent& e,
  ...
  if (auto* enabledComponent = findFirstEnabledAncestor(getParentComponent()))
    enabledComponent->mouseWheelMove(e.getEventRelativeTo(enabledComponent),
                                     wheel);
}
...
void Component::mouseMagnify(const MouseEvent& e, float magnifyAmount) {
  // the base class just passes this event up to the nearest enabled ancestor
  if (auto* enabledComponent = findFirstEnabledAncestor(getParentComponent()))
    enabledComponent->mouseMagnify(e.getEventRelativeTo(enabledComponent),
                                   magnifyAmount);
}
...
void Component::postCommandMessage(int commandID) {
  MessageManager::callAsync(
      [target = WeakReference<Component>{this}, commandID] {
        if (target != nullptr) target->handleCommandMessage(commandID);
      });
}
...
void Component::addMouseListener(MouseListener* newListener,
  ...
  // If you register a component as a mouselistener for itself, it'll receive
  // all the events twice - once via the direct callback that all components get
  // anyway, and then again as a listener!
  jassert((newListener != this) || wantsEventsForAllNestedChildComponents);

  if (mouseListeners == nullptr) mouseListeners.reset(new MouseListenerList());

  mouseListeners->addListener(newListener,
...
//==============================================================================
void Component::internalMouseEnter(MouseInputSource source,
                                   Point<float> relativePos, Time time) {
  ...
  if (flags.repaintOnMouseActivityFlag) repaint();

  const auto me =
      makeMouseEvent(source, detail::PointerState().withPosition(relativePos),
                     source.getCurrentModifiers(), this, this, time,
                     relativePos, time, 0, false);
  ...
  Desktop::getInstance().getMouseListeners().callChecked(
      checker, [&](MouseListener& l) { l.mouseEnter(me); });
  MouseListenerList::sendMouseEvent(checker, &MouseListener::mouseEnter);
}

void Component::internalMouseExit(MouseInputSource source,
                                  Point<float> relativePos, Time time) {
  if (isCurrentlyBlockedByAnotherModalComponent()) {
    // if something else is modal, always just show a normal mouse cursor
    source.showMouseCursor(MouseCursor::NormalCursor);
  ...
  Desktop::getInstance().getMouseListeners().callChecked(
      checker, [&](MouseListener& l) { l.mouseExit(me); });
  MouseListenerList::sendMouseEvent(checker, &MouseListener::mouseExit);
}

void Component::internalMouseDown(
    MouseInputSource source, const detail::PointerState& relativePointerState,
    Time time) {
  auto& desktop = Desktop::getInstance();

  const auto me =
      makeMouseEvent(source, relativePointerState, source.getCurrentModifiers(),
                     this, this, time, relativePointerState.position, time,
  ...
  if (isCurrentlyBlockedByAnotherModalComponent()) {
    ...
    // If processing the input attempt has exited the modal loop, we'll allow
    // the event to be delivered..
    if (isCurrentlyBlockedByAnotherModalComponent()) {
      // allow blocked mouse-events to go to global listeners..
      desktop.getMouseListeners().callChecked(checker, [&](MouseListener& l) {
        l.mouseDown(checker.eventWithNearestParent());
      });
      return;
    }
  }
...
void Component::internalMouseUp(
    MouseInputSource source, const detail::PointerState& relativePointerState,
    Time time, const ModifierKeys oldModifiers) {
  if (flags.mouseDownWasBlocked && isCurrentlyBlockedByAnotherModalComponent())
    return;

  const auto me = makeMouseEvent(
      source, relativePointerState, oldModifiers, this, this, time,
      getLocalPoint(nullptr, source.getLastMouseDownPosition()),
      source.getLastMouseDownTime(), source.getNumberOfMultipleClicks(),
  ...
  auto& desktop = Desktop::getInstance();
  desktop.getMouseListeners().callChecked(checker, [&](MouseListener& l) {
    l.mouseUp(checker.eventWithNearestParent());
  });
  ...
  if (me.getNumberOfClicks() >= 2) {
    ...
    desktop.mouseListeners.callChecked(checker, [&](MouseListener& l) {
      l.mouseDoubleClick(checker.eventWithNearestParent());
    });
    MouseListenerList::sendMouseEvent(checker,
...
void Component::internalMouseDrag(
  ...
  if (!isCurrentlyBlockedByAnotherModalComponent()) {
    const auto me = makeMouseEvent(
        source, relativePointerState, source.getCurrentModifiers(), this, this,
        time, getLocalPoint(nullptr, source.getLastMouseDownPosition()),
        source.getLastMouseDownTime(), source.getNumberOfMultipleClicks(),
        source.isLongPressOrDrag());
    ...
    Desktop::getInstance().getMouseListeners().callChecked(
        checker, [&](MouseListener& l) {
          l.mouseDrag(checker.eventWithNearestParent());
        });
    MouseListenerList::sendMouseEvent(checker, &MouseListener::mouseDrag);
  }
}

void Component::internalMouseMove(MouseInputSource source,
                                  Point<float> relativePos, Time time) {
  auto& desktop = Desktop::getInstance();
  ...
  if (isCurrentlyBlockedByAnotherModalComponent()) {
    ...
    desktop.getMouseListeners().callChecked(checker, [&](MouseListener& l) {
      l.mouseMove(checker.eventWithNearestParent());
    });
    MouseListenerList::sendMouseEvent(checker, &MouseListener::mouseMove);
  }
}

void Component::internalMouseWheel(MouseInputSource source,
                                   Point<float> relativePos, Time time,
                                   const MouseWheelDetails& wheel) {
  auto& desktop = Desktop::getInstance();
  ...
  if (isCurrentlyBlockedByAnotherModalComponent()) {
    // allow blocked mouse-events to go to global listeners..
    desktop.mouseListeners.callChecked(
        checker, [&](MouseListener& l) { l.mouseWheelMove(me, wheel); });
  } else {
    ...
    desktop.mouseListeners.callChecked(checker, [&](MouseListener& l) {
      l.mouseWheelMove(checker.eventWithNearestParent(), wheel);
    });

    if (!checker.shouldBailOut())
      MouseListenerList::sendMouseEvent(checker, &MouseListener::mouseWheelMove,
                                        wheel);
  }
}

void Component::internalMagnifyGesture(MouseInputSource source,
                                       Point<float> relativePos, Time time,
                                       float amount) {
  auto& desktop = Desktop::getInstance();
  ...
  if (isCurrentlyBlockedByAnotherModalComponent()) {
    // allow blocked mouse-events to go to global listeners..
    desktop.mouseListeners.callChecked(
        checker, [&](MouseListener& l) { l.mouseMagnify(me, amount); });
  } else {
    ...
    desktop.mouseListeners.callChecked(checker, [&](MouseListener& l) {
      l.mouseMagnify(checker.eventWithNearestParent(), amount);
    });

    if (!checker.shouldBailOut())
      MouseListenerList::sendMouseEvent(checker, &MouseListener::mouseMagnify,
                                        amount);
  }
}

void Component::sendFakeMouseMove() const {
  if (flags.ignoresMouseClicksFlag && !flags.allowChildMouseClicksFlag) return;

  auto mainMouse = Desktop::getInstance().getMainMouseSource();

  if (!mainMouse.isDragging()) mainMouse.triggerFakeMove();
}
...
void Component::internalBroughtToFront() {
  ...
  componentListeners.callChecked(checker, [this](ComponentListener& l) {
    l.componentBroughtToFront(*this);
  });
  ...
  if (auto* cm = getCurrentlyModalComponent())
    if (cm->getTopLevelComponent() != getTopLevelComponent())
      ModalComponentManager::getInstance()->bringModalComponentsToFront(
          false);  // very important that this is false, otherwise in Windows,
                   // non-front components can't get focus when another modal
                   // comp is active, and therefore can't receive mouse-clicks
}

//==============================================================================
void Component::focusGained(FocusChangeType) {}
void Component::focusGainedWithDirection(FocusChangeType,
...
void Component::internalKeyboardFocusGain(
    FocusChangeType cause, const WeakReference<Component>& safePointer,
  ...
  if (hasKeyboardFocus(false))
    if (auto* handler = getAccessibilityHandler()) handler->grabFocus();
...
void Component::internalKeyboardFocusLoss(FocusChangeType cause) {
  ...
  if (safePointer != nullptr) {
    if (auto* handler = getAccessibilityHandler()) handler->giveAwayFocus();

    internalChildKeyboardFocusChange(cause, safePointer);
  }
}

void Component::internalChildKeyboardFocusChange(
    FocusChangeType cause, const WeakReference<Component>& safePointer) {
...
void Component::setFocusContainerType(
    FocusContainerType containerType) noexcept {
  flags.isFocusContainerFlag =
      (containerType == FocusContainerType::focusContainer ||
       containerType == FocusContainerType::keyboardFocusContainer);

  flags.isKeyboardFocusContainerFlag =
      (containerType == FocusContainerType::keyboardFocusContainer);
}
...
static Component* findContainer(const Component* child,
  ...
  if (auto* parent = child->getParentComponent()) {
    if ((parent->*isFocusContainer)() ||
        parent->getParentComponent() == nullptr)
      return parent;
...
void Component::takeKeyboardFocus(FocusChangeType cause,
                                  FocusChangeDirection direction) {
  if (currentlyFocusedComponent == this) return;

  if (auto* peer = getPeer()) {
    const WeakReference<Component> safePointer(this);
    peer->grabFocus();

    if (!peer->isFocused() || currentlyFocusedComponent == this) return;
...
void Component::grabKeyboardFocusInternal(FocusChangeType cause,
                                          bool canTryParent,
                                          FocusChangeDirection direction) {
...
void Component::grabKeyboardFocus() {
  ...
  // added the component to a parent or made it visible. Or maybe one of its
  // parent components isn't yet visible.
    ...
    if (auto traverser = createKeyboardFocusTraverser()) {
      auto findComponentToFocus = [&]() -> Component* {
        ...
        if (auto* focusContainer = findKeyboardFocusContainer()) {
          auto allFocusableComponents =
              traverser->getAllComponents(focusContainer);
      ...
      if (auto* nextComp = findComponentToFocus()) {
        if (nextComp->isCurrentlyBlockedByAnotherModalComponent()) {
          ...
          if (nextCompPointer == nullptr ||
              nextComp->isCurrentlyBlockedByAnotherModalComponent())
            return;
        }

        nextComp->grabKeyboardFocusInternal(
            focusChangedByTabKey, true,
...
void Component::setEnabled(bool shouldBeEnabled) {
  if (flags.isDisabledFlag == shouldBeEnabled) {
    ...
    // if any parent components are disabled, setting our flag won't make a
    // difference, so no need to send a change message
    if (parentComponent == nullptr || parentComponent->isEnabled())
      sendEnablementChangeMessage();

    BailOutChecker checker(this);
    componentListeners.callChecked(checker, [this](ComponentListener& l) {
      l.componentEnablementChanged(*this);
    });
  ...
  for (auto& ms : Desktop::getInstance().getMouseSources()) {
    ...
    if (c != nullptr && (c == this || (includeChildren && isParentOf(c))))
      if (ms.isDragging() || !(ms.isTouch() || ms.isPen()))
        if (c->reallyContains(c->getLocalPoint(nullptr, ms.getScreenPosition()),
                              false))
          return true;
  }
...
bool Component::isMouseOverOrDragging(bool includeChildren) const {
  for (auto& ms : Desktop::getInstance().getMouseSources()) {
    ...
    if (c == this || (includeChildren && isParentOf(c)))
      if (ms.isDragging() || !ms.isTouch()) return true;
  }
...
void Component::addKeyListener(KeyListener* newListener) {
  if (keyListeners == nullptr) keyListeners.reset(new Array<KeyListener*>());
...
//==============================================================================
void Component::setTitle(const String& newTitle) { componentTitle = newTitle; }
...
void Component::setAccessible(bool shouldBeAccessible) {
  ...
  if (flags.accessibilityIgnoredFlag) invalidateAccessibilityHandler();
}
...
std::unique_ptr<AccessibilityHandler> Component::createAccessibilityHandler() {
  return std::make_unique<AccessibilityHandler>(*this,
                                                AccessibilityRole::unspecified);
}

std::unique_ptr<AccessibilityHandler>
Component::createIgnoredAccessibilityHandler(Component& comp) {
  return std::make_unique<AccessibilityHandler>(comp,
...
AccessibilityHandler* Component::getAccessibilityHandler() {
  if (!isAccessible() || getWindowHandle() == nullptr) return nullptr;
  ...
  if (accessibilityHandler == nullptr ||
    ...
    // On Android, notifying that an element was created can cause the system to
    // request the accessibility node info for the new element. If we're not
    // careful, this will lead to recursive calls, as each time an element is
    // created, new node info will be requested, causing an element to be
    // created, causing a new info request... By assigning the accessibility
    // handler before notifying the system that an element was created, the if()
    // predicate above should evaluate to false on recursive calls, terminating
    // the recursion.
    if (accessibilityHandler != nullptr)
      detail::AccessibilityHelpers::notifyAccessibilityEvent(
          *accessibilityHandler,
          detail::AccessibilityHelpers::Event::elementCreated);
    else
